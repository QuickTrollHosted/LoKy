...
Lit les trames et decode les groupes :   |<--  Switches d'option   -->|
 ...
 PAPP   : index heures pleines en Wh.....|      |   X  |       |      |
...
const uint8_t CpinRx_def = 6;
const uint8_t CpinTx_def = 3;
...
class LinkyHistTIC
  ...
  public:
    ...
    void StopListen();  /* Stop reading process to send value */ // Added by HO Vu Duy Bao
    ...
    bool PAPPIsNew();   /* Returns true if PAPP has changed */
    uint32_t PAPP();    /* Index heures pleines en Wh */
  ...
  private:
    ...
    uint32_t _PAPP;      /* Index heures pleines en Wh */
    uint8_t  _ptec;      /* Periode tarifaire en cours : 0 = HP ; 1 = HC */
    #endif
